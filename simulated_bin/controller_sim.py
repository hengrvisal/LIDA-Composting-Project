...
try:
    from .mpc import choose_action  # type: ignore
    from .models import Forecaster  # type: ignore
    from .features import build_feature_row  # type: ignore
    HAVE_MPC = True
    print(
        "[controller_sim] INFO: Loaded MPC modules via relative imports (.mpc/.models/.features).",
        file=sys.stderr,
    )
except Exception:
